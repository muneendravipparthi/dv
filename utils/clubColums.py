...
df = pd.read_excel('/Users/cb-muneendra/Desktop/headrushtech_subscriptions_expected.xlsx')
df = df[
    ["customer[id]", "subscription[id]", "subscription_items[item_price_id][0]", "subscription_items[item_price_id][1]",
     ...
     "subscription_items[item_price_id][6]"]]
...
df['subscription_items[item_price_id]'] = df.apply(lambda x: '%s' % (sorting_strings(
    [str(x['subscription_items[item_price_id][0]']), str(x['subscription_items[item_price_id][1]']),
     str(x['subscription_items[item_price_id][2]']), str(x['subscription_items[item_price_id][3]']), str(x[
         'subscription_items[item_price_id][4]']), str(x['subscription_items[item_price_id][5]']), str(x['subscription_items[item_price_id][6]'])])), axis=1)
df1 = df[["customer[id]", "subscription[id]", "subscription_items[item_price_id]"]]
...
print(df['merge1'])
df.to_excel("headrushtech_subscriptions_expected3.xlsx", index=False)
